...
public:
	bool raw; 
public:
	void end(ushort ret, const string& desc); 
	bool end(shared_ptr<XscProtoPdu> pdu); 
	XmsgImTransInitiative(shared_ptr<XscChannel> channel, function<void(SptrXiti trans)> cb);
	virtual ~XmsgImTransInitiative();
private:
	function<void(SptrXiti trans)> cb; 
};
